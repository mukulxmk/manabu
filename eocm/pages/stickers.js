...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in stickerss) {
      if (!stickerss[item.title].color.includes(item.color) && item.availableQty > 0) {
        stickerss[item.title].color.push(item.color)
      }
      if (!stickerss[item.title].size.includes(item.size) && item.availableQty > 0) {
        stickerss[item.title].size.push(item.size)
      }
      ...
      stickerss[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        stickerss[item.title].color = [item.color]
        stickerss[item.title].size = [item.size]
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(stickerss)) }
  }
}
